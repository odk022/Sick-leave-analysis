...
Alle tall er hentet fra statistikkbanken i Statistisk sentralbyrå.
...
Figuren viser at det egenmeldte fraværet er konstant over tid, og at det totale fraværet har variert over tid. Det egenmeldte fraværet er sett vekk fra i det følgende.

### Kvinner, alle typer sykefravær, alle næringer

```{r}
#| label: oversikt_kvinner
...
Vi ser av figuren at det legemeldte sykefraværet for kvinner har økt de siste årene og var i 2021 på `r Year_2021_kv$sykefraværsprosent` $\%$. Gjennomsnittet for hele 20-årsperioden var `r round(mean(gjsn_kv$sykefraværsprosent),1)` $\%$.
...
Vi ser av figuren at det legemeldte sykefraværet for menn har hatt en nedgang i mange år, men økte fra 2019 til 2020. Sykefraværet var i 2021 på `r Year_2021_m$sykefraværsprosent` $\%$. Gjennomsnittet for hele 20-årsperioden var `r round(mean(gjsn_m$sykefraværsprosent),1)` $\%$.

### Legemeldt fravær fordelt på kjønn

```{r}
#| label: oversikt_totalt_kjønn
...
Figuren viser at det er stor forskjell i nivå på legemeldt sykefravær mellom kjønn. Det er derfor grunn til å spørre om det er ulike årsaker til fraværet mellom kjønnene som forårsaker denne forskjellen, og hvis det er slik, hvilke årsaker er det tale om.
