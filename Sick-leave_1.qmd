---
title: "Sykefravær i Norge og i Troms og Finnmark"
author: "Oddmund"
toc: true
toc-title: Innhold
...
### Troms og Finnmark kommunefordelt

```{r}
tabell_hel<- ApiData("https://data.ssb.no/api/v0/no/table/12448/", 
    Region=list('agg_single:Komm2020', c("5401", "5402", "5403", "5404", "5405", "5406", "5411", "5412", "5413", "5414", "5415", "5416", "5417", "5418", "5419", "5420", "5421", "5422", "5423", "5424", "5425", "5426", "5427", "5428", "5429", "5430", "5432", "5433", "5434", "5435", "5436", "5437", "5438", "5439", "5440", "5441", "5442", "5443", "5444")), 
Kjonn=list('item', c("0", "1", "2")), 
Alder=list('item', c("00")), 
ContentsCode=list('item', c("Sykefraversprosent")), 
Tid=list('item', c("2020K1", "2020K2", "2020K3", "2020K4", "2021K1", "2021K2", "2021K3", "2021K4", "2022K1", "2022K2", "2022K3", "2022K4", "2023K1", "2023K2", "2023K3")))

data_kommune<- tabell_hel[[1]]
#glimpse(data_kommune)
```

```{r}
# Rydder i datasettet
data_kommune<- data_kommune %>% 
  rename("kommune" = "region", "sykefraværsprosent" = "value") %>% 
  select(-c("statistikkvariabel", "alder"))
...
```{r Tromsø kjønn}
#| label: Tromsø kjønn
#| fig-cap: Sykefravær i Tromsø fordelt på kjønn
#| fig-width: 12
#| fig-height: 6


Tromsø_kj<-data_kommune %>% 
filter(kommune =="Tromsø")
# setter inn kommunenavn i filteret. Hvis flere kommuner, brukes 
#filter(kommune %in% c("kommunenavn1", "kommunenavn2")) 
Tromsø<- 
  ggplot(TogF_kj,aes(x=kvartal,y=sykefraværsprosent, group=kjønn))+
geom_line(aes(color= kjønn), linewidth = 1.0)+
  #geom_point() +
  ylim(0, 15)+
  labs(title = "Sykefravær i Tromsø etter kjønn", 
       x = "Kvartal", y = "Sykefraværsprosent") +
   theme(plot.title = element_text(size = 18, face = "bold")) +
   theme(axis.text=element_text(size=11),
        axis.title=element_text(size=11,face="bold")) + 
  theme(legend.text=element_text(size=12),
        ...
        # )
Tromsø
# lage små plott for hver kvartalsgruppe
```

```{r}
#| label: kommuneoversikt
kommuner_kvartal<- data_kommune %>% 
  filter(kjønn == "Begge kjønn") %>%
  pivot_wider(names_from = `kvartal (u)`, values_from = sykefraværsprosent) 
```

```{r}
#| label: tbl-kommuner
#| tbl-cap: "Sykefravær totalt fordelt på kommuner"


tabell_kommuner_alle <- 
  kommuner_kvartal %>% 
  select(-c("2020K1":"2020K4", "kjønn")) 

kable(tabell_kommuner_alle) 
```
